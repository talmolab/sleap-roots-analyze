...
- [x] 1.1 Add `cross_platform_planted_signal_fixture` to `tests/fixtures.py` — **redesigned during
      ...
      order.
- [x] 1.2 Add `cross_platform_pure_noise_fixture` to `tests/fixtures.py` — same N=20-seed
...
- [x] 1.3 Add `cross_platform_synthetic_non_edpie_fixture` to `tests/fixtures.py` — a fixture
...
- [x] 2.1 Write failing test `test_fit_pca_on_fold_fits_on_train_only`: build `X_train`,
...
- [x] 2.2 Write failing test `test_fit_pca_on_fold_output_shape`: `X_train` shape
      `(n_train, n_traits)`, `X_test` shape `(n_test, n_traits)`, assert output shape is
      `(n_test, n_components)` for `n_components in (1, 2)`.
- [x] 2.3 Write failing test `test_fit_pca_on_fold_raises_when_n_traits_less_than_n_components`:
...
- [x] 2.4 Write failing test `test_fit_pca_on_fold_deterministic`: call twice with identical
      `X_train`/`X_test`/`n_components=1`; assert identical output (no random-state sensitivity
      for a full-rank `X_train`).
- [x] 2.5 Write failing test `test_fit_pca_on_fold_does_not_mutate_inputs`: assert `X_train`/
      `X_test` arrays are unchanged after the call (no in-place sklearn side effects leak
      through).
- [x] 2.6 Implement `fit_pca_on_fold(X_train, X_test, n_components=1) -> np.ndarray` in new module
...
- [x] 3.1 Write failing test `test_logo_cv_predict_pipeline_instantiated_inside_fold`
...
- [x] 3.2 Write failing test `test_logo_cv_predict_representatives_scaler_never_sees_held_out_genotype`:
...
- [x] 3.3 Write failing test `test_logo_cv_predict_pls_latent_uses_fixed_n_components_1`: assert
...
- [x] 3.4 Write failing test `test_logo_cv_predict_pls_latent_never_sees_held_out_genotype_y`:
...
- [x] 3.5 Write failing test `test_logo_cv_predict_representative_names_fixed_pre_loop`:
...
- [x] 3.6 Write failing test `test_logo_cv_predict_pc1_calls_fit_pca_on_fold_per_fold`:
...
- [x] 3.7 Write failing test `test_logo_cv_predict_returns_one_prediction_per_genotype`: output
      length equals `len(genotypes)`, in the same order as the input `genotypes` sequence.
- [x] 3.8 Write failing test `test_logo_cv_predict_planted_signal_recovers_expected_r2`: using
      ...
      reliably.
- [x] 3.9 Write failing test `test_logo_cv_predict_pure_noise_r2_clearly_below_signal`: using
...
- [x] 3.10 Write failing test `test_logo_cv_predict_synthetic_non_edpie_fixture_generalizes`:
...
- [x] 3.11 Write failing test `test_logo_cv_predict_computes_rmse_and_spearman_rho`: assert RMSE
      and Spearman ρ (with its p-value) are returned alongside R², all three computed over the
      same concatenated predictions.
- [x] 3.13 Write failing test `test_logo_cv_predict_rejects_mismatched_lengths`: `len(X) !=
      len(y)` or `len(X) != len(genotypes)` — assert a clean `ValueError`, not an obscure
      downstream shape error.
- [x] 3.14 Write failing test `test_logo_cv_predict_rejects_invalid_reduction_method`:
      `reduction_method="not_a_real_method"` — assert `ValueError` listing the three valid values.
- [x] 3.15 Write failing test `test_logo_cv_predict_representatives_requires_representative_names`:
...
- [x] 3.18 Write failing test `test_logo_cv_predict_rejects_nan_in_X`: `X` containing a `NaN`
...
- [x] 3.19 Implement `logo_cv_predict(X, y, genotypes, reduction_method="pls_latent",
...
- [x] 4.1 Write failing test `test_leakage_detectable_ratio_at_least_1_10`
...
- [x] 4.2 Write failing test `test_leakage_not_falsely_flagged_on_pure_noise`: same
...
- [x] 4.3 Ensure production `logo_cv_predict` (Section 3) has no code path equivalent to the
...
- [x] 5.1 Write failing test `test_cluster_and_correlate_reproduces_section_3_4_representative_counts`
...
- [x] 5.2 Write failing test `test_cross_platform_correlation_filter_reproduces_section_3_4_trait_set`:
...
- [x] 5.3 Write failing test `test_cluster_representatives_deterministic_given_same_input`: run
...
- [x] 6.1 Write failing test `test_cross_platform_prediction_result_round_trips_through_json`:
      ...
      contract).
- [x] 6.2 Write failing test `test_cross_platform_prediction_result_no_sklearn_objects`: assert
      no sklearn/numpy object appears in `dataclasses.asdict(result)`.
- [x] 6.3 Write failing test `test_cross_platform_prediction_result_from_logo_cv_adapter`: an
...
- [x] 6.4 Write failing test `test_cross_platform_prediction_result_pc1_reported_separately`:
...
- [x] 6.5 Implement `CrossPlatformPredictionResult` and `TargetPrediction` in `result_types.py`
      ...
      Make 6.1-6.4 green.
- [x] 6.6a **(New — added after `/review-openspec` round 1)** Write failing test
...
- [x] 6.6b Export `CrossPlatformPredictionResult` and `TargetPrediction` from
...
- [x] 7.1 Write failing test `test_cross_platform_prediction_functions_importable_from_package_root`
...
- [x] 7.2 Add both names to `__all__` in `sleap_roots_analyze/__init__.py`, in the **same
...
- [x] 8.1 **Manual, not part of `pytest`.** Built real BLUP tables for all 4 EDPIE platforms
      (Turface19, Turface150, Cylinder, Field) via direct Python API calls
      (`calculate_heritability_estimates` + `extract_blup_table`) on the real post-QC data
      already committed at `tests/fixtures/real/wheat_edpie/inputs/post_qc/` — the Feb-12 vintage
      for Turface19/Turface150, and the Mar-30 paper vintage (task 1.4's regenerated files) for
      Cylinder/Field, since that's the more authoritative vintage for those two platforms. Did not
      go through the full config-driven `StatisticalAnalysisStep` pipeline (no real platform
      configs were needed/supplied) — the direct function calls produce the same real BLUP
      substrate.
- [x] 8.2 **Manual, not part of `pytest`.** Ran `logo_cv_predict` for the 4 directed pairs with
      both `reduction_method="pls_latent"` and `"representatives"`, against each target
      platform's top-5-variance cluster-representative traits plus PC1 (32 target/method/pair
      combinations total). Additionally ran a **targeted follow-up**: the 3 specific bivariate
      trait-pair associations named in the paper's Section 3.4.2
      (`Lateral Lengths P25 P5`→`Stmdw M Gm2`, `Seminal Angles Distal Min P95`→`Spkdw Calc Gm2`,
      `Seminal Angles Distal Std P95`→`Gw M G1000Grn`) as single-column predictors — a much more
      direct test than the full noisy trait matrix. All artifacts (summary CSV, 4 PC1 scatter
      plots, 3 named-pair scatter plots) saved to a scratchpad directory, not committed to the
      repo.
- [x] 8.3 **Manual, not part of `pytest`.** Findings:
      - **Named-pair recovery (strong positive result):** LOGO-CV cleanly recovers all 3 of the
        paper's own strongest published bivariate correlations as real predictive signal — R²=
        +0.41/+0.08/+0.05, ρ=+0.66/+0.53/+0.51 (all p<0.03) — **regardless of whether the
        published correlation itself was negative (-0.744, -0.618) or positive (+0.656)**. A
        correctly-fit model's predictions track the truth (positive y_pred-vs-y_true R²/ρ)
        whether the underlying trait-trait relationship is positively or negatively signed; the
        sign gets absorbed into the fitted coefficient. This is a strong, direct validation that
        the machinery works correctly on known real signal.
      - **Full-trait-matrix / PC1 results are noisier:** all 4 pairs' PC1 predictions (via
        PLS/Ridge over the ~10-20-trait full representative set) show near-zero-to-negative R²,
        and 2 of 4 pairs show a significant *negative* y_pred-vs-y_true Spearman ρ
        (Turface19→Cylinder: -0.62, p=0.005; Cylinder→Field: -0.48, p=0.038). Initially
        flagged as a possible artifact; the named-pair follow-up clarified this is better
        explained as a curse-of-dimensionality symptom (many noisy/collinear predictors, n=19
        training genotypes) than a code defect or a genuine recovered negative relationship — a
        real property of applying PLS/Ridge over the full trait matrix at this sample size, not
        something this tier's code should paper over.
      - **Cross-checked the regenerated `cross_platform_correlations.csv` (task 1.4) against all
        3 named pairs independently of the LOGO-CV work above:** reproduces the paper's published
        ρ/p almost exactly (-0.744 vs -0.74, p=0.000261 vs 0.0003; +0.656 vs +0.66, p=0.0023 vs
        0.002; -0.618 vs -0.62, p=0.0048 vs 0.005) — further confirmation the Decision 2 fixture
        regeneration is correct, independent of the trait-set-count check in Section 5.
      - No implementation bug found; no design-level change made to this tier as a result.
      **Elizabeth reviewed these findings and signed off 2026-07-16** — gates PR readiness
      alongside `/pre-merge-check`, per the roadmap's per-tier loop.

## 9. Docs

- [x] 9.1 Add `## cross_platform_prediction Module` section to `docs/API.md`, **including its
...
- [x] 9.2 Add a `docs/CHANGELOG.md` `[Unreleased]` `### Added` entry: `fit_pca_on_fold()`,
...
- [x] 9.3 Add a `CrossPlatformPredictionResult` row to `docs/result-types.md`, following the
...
- [x] 9.4 **(New — added after `/review-openspec` round 1)** Add a new section to
...
- [x] 10.1 `openspec validate add-cross-platform-prediction --strict` — resolve every reported
      issue before requesting review.
- [x] 10.2 `/lint` (black + ruff) on all changed files.
- [x] 10.3 Full `uv run pytest --cov --cov-branch` — confirm no regressions, and that all new
...
- [x] 10.5 Complete Section 8's manual real-data validation and get Elizabeth's explicit sign-off
