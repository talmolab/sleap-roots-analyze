...
- [ ] 1.2 Confirm the three baseline lines this fix will resolve are, in
...
- [ ] 2.2 In `_cluster_experiment()` (~L191), change the return-type annotation's third
...
- [ ] 3.1 In `tests/test_trait_redundancy.py`, add
      ...
      coverage gap.
- [ ] 3.2 Run `uv run pytest tests/test_trait_redundancy.py -v` — confirm these
...
- [ ] 5.1 Run `uv run pytest tests/test_trait_redundancy.py -v` (full file, not just the
      two touched tests) — confirm no regressions.
- [ ] 5.2 Run `uv run ruff check src/sleap_roots_analyze` and
      `uv run black --check src/sleap_roots_analyze tests` — confirm clean.

## Task 6: Spec, docs, changelog
- [ ] 6.1 `cli-pipeline` spec delta (this change's `specs/cli-pipeline/spec.md`) — done
      as part of this proposal.
- [ ] 6.2 Add a `### Fixed` entry to `docs/CHANGELOG.md` `[Unreleased]` noting the
...
- [ ] 7.1 In the PR description (not this proposal), note this closes the first checklist
