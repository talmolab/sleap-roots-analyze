...
This spec defines requirements for maintaining reproducible analysis configurations that exactly match the parameters used in research notebooks.
...
### QC Parameters
- **Data Input:** csv_path, image_dir, additional_exclude_cols
- **Column Mappings:** barcode, genotype, replicate
- **Cleanup:** max_nan_fraction, max_zeros_per_trait, max_nans_per_trait, min_samples_per_trait
- **Outlier Detection:** traditional_methods, clustering_methods, mahalanobis settings
- **Outlier Removal:** strategy, method
- **PCA:** n_components, feature_selection_strategy
- **Heritability:** enabled, threshold, generate_diagnostics
- **Custom:** custom_replacements (e.g., crown→seminal)

### Viz Parameters
- **Data Input:** csv_path, image_dir
- **PCA:** n_components, feature_selection_strategy, n_top_features
- **UMAP:** enabled, n_neighbors, min_dist, metric
- **Genotype Highlighting:** genotypes_to_color, highlight_genotypes
- **Figure Settings:** dpi, formats, figsize

### Cross-Platform Parameters
- **Data Sources:** exp1_data_path, exp2_data_path, genotype columns
- **Analysis:** correlation_method, min_samples_per_genotype, significance_level
- **Visualization:** top_n_correlations, top_n_joint_plots, top_n_boxplots

---

## Maintenance Notes

- This spec should be updated when new parameter categories are added to pipelines
- Parameter reference tables should be updated when configs are modified
- Validation checklists should be revised based on lessons learned from audits
- Each dataset config should be re-validated when pipelines are significantly updated
