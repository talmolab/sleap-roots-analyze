...
- [x] 1.1 Write `tests/test_public_api.py` (new) asserting, for each of the 8
...
- [x] 1.2 Add `__all__` hygiene asserts to the same test: no duplicates
...
- [x] 1.4 Add a `# Statistics / heritability functions` section listing all 8
  names in `__all__`
- [x] 1.5 Run `uv run black src/sleap_roots_analyze/__init__.py` to remove the
...
- [x] 2.1 Add `Any` to the `from typing import ...` line in `statistics.py`
- [x] 2.2 Add a test asserting `typing.get_type_hints(fn)` succeeds for each of the
  8 functions and that no parameter/return annotation is empty

## 3. Docstrings & module scope

- [x] 3.1 Audit each of the 8 functions for complete Google-style Args/Returns/
...
- [x] 3.2 Expand the `statistics.py` module docstring to describe scope vs
  `cross_experiment_analysis.py` (name it explicitly)
- [x] 3.3 Add a test asserting each function's docstring contains `Args:` and
  `Returns:`, and that `statistics.__doc__` mentions `cross_experiment_analysis`
- [x] 3.4 Run `uv run ruff check` (pydocstyle) to confirm docstrings pass

## 4. Documentation

- [x] 4.1 In `docs/API.md` `## statistics Module`: add the **3** missing entries
...
- [x] 4.2 Add a test asserting `docs/API.md` contains each of the 8 function names
- [x] 4.3 Add an `### Added` entry under `[Unreleased]` in `docs/CHANGELOG.md`
  noting the 8 functions are now importable from `sleap_roots_analyze`

## 5. Validation

- [x] 5.1 `uv run pytest tests/test_public_api.py tests/test_statistics.py` pass
- [x] 5.2 `uv run black --check` + `uv run ruff check` clean
- [x] 5.3 `openspec validate expose-statistics-functions --strict` passes
