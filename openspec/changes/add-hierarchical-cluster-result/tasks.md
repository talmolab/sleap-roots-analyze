...
- [ ] 1.1 Write failing round-trip test: a `HierarchicalResult` built from an inline
...
- [ ] 1.2 Write failing test: `ClusterResult(random_state=None)` and each subclass
      construct without error, and `dataclasses.asdict(result)["random_state"]` is `None`.
- [ ] 1.3 Write failing test: `from_hierarchical_dict(d)` (with `d` an inline labeled
...
- [ ] 1.4 Write failing test: `HierarchicalResult.to_json()` raises `ValueError` on a
      non-finite `cophenetic_correlation` (mirror `test_to_json_rejects_non_finite_bic`).
- [ ] 1.5 Make `ClusterResult.random_state` `Optional[int] = None` and update the base
...
- [ ] 1.6 Add `ALGORITHM_HIERARCHICAL = "hierarchical"` (append to
...
- [ ] 1.7 Add `ClusterResult.from_hierarchical_dict(d)` adapter (no `random_state`),
      non-mutating, stamping `random_state=None`; make 1.1–1.4 green.

## 2. Public producer entry point (test-first)

- [ ] 2.1 Write failing regression test: `perform_hierarchical_clustering(data)` still
...
- [ ] 2.2 Write failing test: `hierarchical_cluster_labels(df)` returns a dict with
...
- [ ] 2.2b Write failing delegation test (S1's operative guarantee): on the same input,
...
- [ ] 2.3 Write failing test: `hierarchical_cluster_labels(df, n_clusters=3)` yields
      `n_clusters == 3`, `len(cluster_sizes) == 3`, and
      `sum(cluster_sizes) == len(cluster_labels)`.
- [ ] 2.4 Write failing determinism test: two calls on identical input produce
      identical `cluster_labels` (same-process exact equality — **not** a committed
      golden; see design.md).
- [ ] 2.5 Write failing error-path tests: `hierarchical_cluster_labels` propagates
...
- [ ] 2.5b Write failing test for the `optimization_method` surface: each accepted value
...
- [ ] 2.6 Write failing degenerate test: `n_clusters=1` yields `n_clusters == 1`,
...
- [ ] 2.6b Write failing boundary error-contract test pinning the ACTUAL exception type
...
- [ ] 2.7 Implement `hierarchical_cluster_labels()` composing
      ...
      make 2.2–2.6b green.
- [ ] 2.8 Write end-to-end test then confirm green:
...
- [ ] 3.2 Add `HierarchicalResult` and `hierarchical_cluster_labels` (only — **not**
      the constant) to `__init__.py` and `__all__`; make 3.1 green.
- [ ] 3.3 Update `docs/result-types.md`: add the hierarchical row to the types table
      ...
      drift.
- [ ] 3.4 Add a `docs/CHANGELOG.md` `[Unreleased]` entry: `### Added`
...
- [ ] 4.1 `openspec validate add-hierarchical-cluster-result --strict` (CLI not
      currently installed — if unavailable, hand-check format against `openspec/AGENTS.md`).
- [ ] 4.2 `/lint` + full pytest + coverage via `/pre-merge-check`.
