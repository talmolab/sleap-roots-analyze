...
```
[1/4] Black formatting check...
would reformat src/sleap_roots_analyze/cross_experiment_analysis.py
1 file would be reformatted, 22 files would be left unchanged.
FAILED

Fix: Run 'uv run black src/sleap_roots_analyze tests' to auto-fix formatting
```
...
- **Black fails**: `uv run black src/sleap_roots_analyze tests`
- **Ruff fails**: `uv run ruff check --fix src/sleap_roots_analyze` (then fix remaining manually)
...
- **Lint job step 1**: `uv run black --check src/sleap_roots_analyze tests`
- **Lint job step 2**: `uv run ruff check src/sleap_roots_analyze`
- **Test job**: `uv run pytest --cov=src/sleap_roots_analyze --cov-report=xml --durations=-1 tests/`
...
|-------|-------------|------------|--------|
| Black | `uv run black --check src/sleap_roots_analyze tests` | Same | Yes |
| Ruff | `uv run ruff check src/sleap_roots_analyze` | Same | Yes |
