...
uv run pytest --cov=src/sleap_roots_analyze --cov-report=term-missing --cov-report=html tests/

# XML report matching CI
uv run pytest --cov=src/sleap_roots_analyze --cov-report=xml --durations=-1 tests/

# Fail if coverage drops below 80%
uv run pytest --cov=src/sleap_roots_analyze --cov-report=term-missing --cov-fail-under=80 tests/

# Coverage for specific test file
uv run pytest --cov=src/sleap_roots_analyze --cov-report=term-missing tests/test_cross_experiment_analysis.py
```
