...
unset GITHUB_TOKEN && gh api repos/talmolab/sleap-roots-analyze/pulls/<PR_NUMBER>/reviews \
  --jq '.[] | {author: .user.login, state: .state, body: .body}'
unset GITHUB_TOKEN && gh api repos/talmolab/sleap-roots-analyze/pulls/<PR_NUMBER>/comments \
...
**Repo root:** `c:/repos/sleap-roots-analyze`
